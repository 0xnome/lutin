...
Expression *ExpressionDiv::simplifier(TableDesSymboles* tableDesSymboles) {
    if(this->terme->estConstante(tableDesSymboles)){
        // 1 / facteur = 0  (div entiere) NON, pas dans le case où facteur = 1, oui sinon TODO ?
        // 0 / facteur = 0 (absorbant)
        if(/*this->terme->eval(tableDesSymboles) == ExpressionDiv::ELEMENT_NEUTRE
           || */this->terme->eval(tableDesSymboles) == ExpressionDiv::ELEMENT_ABSORBANT){
            Expression* exp = (Expression*) new ConstanteNumerique(new NumTerminal(0, this->facteur->getLigne(), this->facteur->getColonne()));
            return exp;
        }
    }
    // terme / 1 = terme
    else if(this->facteur->estConstante(tableDesSymboles) && this->facteur->eval(tableDesSymboles) == ExpressionDiv::ELEMENT_NEUTRE){
